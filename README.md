...
For Ubuntu-like environments, the repository now provides a local one-command
build flow from the project root.

Install dependencies:

```shell
sudo apt-get install -y scons emscripten python3-flask nodejs npm
```

Then build everything from the repository root:
...
- build `build/stellarium-web-engine.js`
- build `build/stellarium-web-engine.wasm`
- copy the engine files into `apps/web-frontend/src/assets/js/`
- install frontend dependencies
- build `apps/web-frontend/dist/`

To start the local web service after the build:
...
> [!NOTE]
> The local frontend build path currently uses `/usr/bin/node` and applies
> `NODE_OPTIONS=--openssl-legacy-provider` for webpack 4 compatibility.
> If `npm ci` cannot proceed because the lockfile is not fully in sync with the
> current npm workspace resolution, the local build flow automatically falls
> back to `npm install`.
